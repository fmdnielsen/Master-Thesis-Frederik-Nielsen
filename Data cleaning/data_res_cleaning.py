...
#%% Loading data
df_el = pd.read_csv("Meterdata.csv",sep=';')
...
hourly_df['date'] = pd.to_datetime(hourly_df['HourDK'], format='%Y-%m-%d %H:%M')
columns_to_drop = [0]
hourly_df.drop(df_el.columns[columns_to_drop], axis=1, inplace=True)


# Assuming 'date_time' is the column containing dates and times
# Convert 'date_time' to datetime type if it's not already
hourly_df['date'] = pd.to_datetime(hourly_df['date'])

# Generate a complete date range from the minimum to maximum date in the DataFrame
complete_range = pd.date_range(start=hourly_df['date'].min(),
                              end=hourly_df['date'].max(),
                              freq='H')

# Convert the complete date range to a DataFrame
complete_df = pd.DataFrame({'date': complete_range})

# Merge the complete date range DataFrame with the original DataFrame
hourly_df = pd.merge(complete_df, hourly_df, on='date', how='left').fillna(0)

# Set 'date_time' as the index if needed
hourly_df["ConsumptionkWh"] = hourly_df["ConsumptionkWh"].str.split(',').str[0]
hourly_df["ConsumptionkWh"] = hourly_df["ConsumptionkWh"].astype(float)
hourly_df = hourly_df.set_index('date')
remove = hourly_df.index.duplicated(keep='first')
hourly_df = hourly_df.drop(hourly_df.index[remove])
#hourly_df = hourly_df.drop_duplicates()
#hourly_df = hourly_df.resample('H').mean()


# Remove data after comma in 'column1'
...
#%% Save data
hourly_df.to_csv('data_7_clean.csv')
